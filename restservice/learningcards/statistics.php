...
if (!empty($ilpath)) {
    ...
    switch($request_method) {
        ...
        case "PUT":
            ...
            logging("statistics.php post/put request " . $userId);
            if ($userId > 0) {
    ...
    //		logging(" statistics data" . $statistics);
        ...
        case "GET":
            ...
            logging("statististics.php get request");
    // 		$userId = get_session_user_from_headers();
            if ($userId > 0) {
                logging("has valid user");
...
// 			logging("new ID: " . $myID);
 	             	logging('add record '.json_encode($statisticItem));
